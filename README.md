## DESCRIPTION social-net-backend
Бэкенд социальной сети на java. Пользователи имеют возможность:
...
Проект состоит из 3-х java микросервисов (user-service, gateway-service, communication-service), и фронтенд (расположен в ./frontend/ как сабмодуль данного репозитория).  
Для хранения данных пользователей используется СУБД postgre. Для миграций БД в проекте используется liquibase (выведен в отдельный сервис db-managment, запускается во время build при deploy проекта или локально mvn clean install).  
Микросервисы используют jooq для выполнения запросов к бд.  
Логирование сервисов реализлванно на aop.  
Каждый микросервис пакуется в docker отдельный контейнер.  
В проекте есть docker-compose.yml файл для запуска контейнеров на сервере (для локального билда требуется указать параметры, если запускать сервисы в ide, то нужен только postgre, frontend можно сбилдить локально по README в ./frontend/).  

## Демонстрация
<img src="./.documentation/demo1.gif" width="1200" height="800" alt="Демонстрация gif"/>
